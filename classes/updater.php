...
if ( ! class_exists( 'PMAI_Updater' ) ) {
	...
	class PMAI_Updater {
		...
		function check_update( $_transient_data ) {
			...
			if ( empty( $_transient_data->response ) || empty( $_transient_data->response[ $this->name ] ) ) {
				...
				if ( false === $version_info ) {
					...
					$version_info = $this->api_request( 'check_update', [ 'slug' => $this->slug ] );
		...
		public function show_update_notification( $file, $plugin ) {
			...
			remove_filter( 'pre_set_site_transient_update_plugins', [ $this, 'check_update' ], 21 );
		...
		private function api_request( $_action, $_data ) {
			...
			$request = wp_remote_post( $this->api_url, [ 'timeout'   => 15,
			                                             'sslverify' => false,
			                                             'body'      => $api_params,
			] );
		...
		public function show_changelog() {
			...
			if ( ! current_user_can( 'update_plugins' ) ) {
				wp_die( __( 'You do not have permission to install plugin updates', 'edd' ), __( 'Error', 'edd' ), [ 'response' => 403 ] );
			}

			$response = $this->api_request( 'show_changelog', [ 'slug' => $_REQUEST['slug'] ] );
