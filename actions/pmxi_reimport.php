...
function pmai_pmxi_reimport( $entry, $post ) {
    ...
    <div class="input">
        ...
        <input type="checkbox" id="is_update_mb_<?= $entry; ?>" name="is_update_mb"
               value="1" <?= $post['is_update_mb'] ? 'checked="checked"' : '' ?> class="switcher"/>
        <label for="is_update_mb_<?= $entry; ?>"><?php _e( 'Meta Box', 'mb-wpai' ) ?></label>
        <div class="switcher-target-is_update_mb_<?= $entry; ?>" style="padding-left:17px;">
            <div class="input">
                <input type="radio" id="update_mb_logic_full_update_<?= $entry; ?>" name="update_mb_logic"
                       ...
                       class="switcher"/>
                <label for="update_mb_logic_full_update_<?= $entry; ?>"><?php _e( 'Update all Meta Box fields', 'mb-wpai' ) ?></label>
            </div>
            <div class="input">
                <input type="radio" id="update_mb_logic_mapped_<?= $entry; ?>" name="update_mb_logic"
                       ...
                       class="switcher"/>
                <label for="update_mb_logic_mapped_<?= $entry; ?>"><?php _e( 'Update only mapped Meta Boxes', 'mb-wpai' ) ?></label>
            </div>
            <div class="input">
                <input type="radio" id="update_mb_logic_only_<?= $entry; ?>" name="update_mb_logic"
                       value="only" <?= ( "only" == $post['update_mb_logic'] ) ? 'checked="checked"' : '' ?>
                       class="switcher"/>
                <label for="update_mb_logic_only_<?= $entry; ?>"><?php _e( 'Update only these Meta Box fields, leave the rest alone', 'mb-wpai' ) ?></label>
                <div class="switcher-target-update_mb_logic_only_<?= $entry; ?> pmxi_choosen"
                     style="padding-left:17px;">

                    <span class="hidden choosen_values"><?php if ( ! empty( $all_field_ids ) ) {
							echo implode( ',', $all_field_ids );
						} ?></span>
            ...
            <div class="input">
                <input type="radio" id="update_mb_logic_all_except_<?= $entry; ?>" name="update_mb_logic"
                       ...
                       class="switcher"/>
                <label for="update_mb_logic_all_except_<?= $entry; ?>"><?php _e( 'Leave these Meta Box fields alone, update all other Meta Box fields', 'mb-wpai' ) ?></label>
