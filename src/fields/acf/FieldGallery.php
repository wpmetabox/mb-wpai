...
class FieldGallery extends Field {
    ...
    public function getFieldValue() {
        ...
        $gallery_ids = $is_append_new ? MetaboxService::get_post_meta($this, $this->getPostID(), $this->getFieldName()) : array();
        ...
        if (!empty($values[$this->getPostIndex()])) {
            ...
            foreach ($values[$this->getPostIndex()] as $url) {
                if ("" != $url and $attid = MetaboxService::import_image(trim($url), $this->getPostID(), $parsingData['logger'], $search_in_gallery, $search_in_files, $this->importData) and !in_array($attid, $gallery_ids)) {
                    $gallery_ids[] = $attid;
                }
            }
        }
