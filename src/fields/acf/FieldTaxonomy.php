...
class FieldTaxonomy extends Field {
    ...
    public function import($importData, $args = array()) {
            ...
            if (!empty($values)) {
                ...
                foreach ($values as $tx_name => $txes) {
                    ...
                    if (!empty($assign_taxes)) {
                        ...
                        MetaboxService::update_post_meta($this, $this->getPostID(), $this->getFieldName(), $value);
                    }
                }
            }
        }
        elseif ($parsedData['is_multiple']) {
            ...
            MetaboxService::update_post_meta($this, $this->getPostID(), $this->getFieldName(), $mult_values);
        }
        else {
            $v = is_array($values) ? array_shift($values) : $values;
            MetaboxService::update_post_meta($this, $this->getPostID(), $this->getFieldName(), $v);
        }
    }
    ...
    public function saved_post($importData) {
        ...
        $assign_taxes = MetaboxService::get_post_meta($this, $this->getPostID(), $this->getFieldName());
        ...
        if (!empty($assign_taxes) && !empty($field['save_terms'])){
            ...
            MetaboxService::associate_terms($this->getPostID(), (empty($assign_terms) ? FALSE : $assign_terms), $field['taxonomy'], $this->getLogger());
        }
    }
