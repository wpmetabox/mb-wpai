...
namespace wpai_meta_box_add_on\fields\mb\field_clone;
...
/**
 * Class FieldCloneV5
 * @package wpai_meta_box_add_on\fields\mb\field_clone
 */
class CloneV5 extends Field {
    ...
    public function import($importData, $args = array()) {
        ...
        MetaboxService::update_post_meta($this, $this->getPostID(), $this->getFieldName(), '');
    }
        ...
        if (!empty($field['clone'])){
            foreach ($field['clone'] as $sub_field_key) {
                if (strpos($sub_field_key, 'group_') === 0){
	                $meta_boxes = acf_get_field_groups();
                    if (!empty($meta_boxes)){
                        foreach ($meta_boxes as $acf_group){
