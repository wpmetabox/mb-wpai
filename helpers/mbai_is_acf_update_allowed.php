				...
				foreach ( $mapped_acf as $acf_group_id => $is_mapped ) {
					...
					if ( ! empty( $acf_fields ) ) {
						foreach ( $acf_fields as $meta_key => $cur_meta_val ) {
							...
							if ( ! in_array( $field['name'], $all_acf_fields ) ) {
								$all_acf_fields[] = $field['name'];
							}

							if ( ! empty( $field['sub_fields'] ) ) {
								foreach ( $field['sub_fields'] as $sub_field ) {
									if ( ! in_array( $sub_field['name'], $all_acf_fields ) ) {
										$all_acf_fields[] = $sub_field['name'];
									}
								}
							}
						}
